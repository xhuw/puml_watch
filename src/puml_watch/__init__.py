...
def parse_args(argv):
    ...
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="remove output except from plantuml."
    )
    parser.add_argument(
        "--once",
        action="store_true",
        help="Run plantuml once on all the provided files",
    )
...
def puml_once(puml_jar, paths, out_dir):
    for file in paths:
        do_puml(puml_jar, file, out_dir)
...
def puml_watch(args):
    ...
    try:
        with puml.plantuml_path() as plantuml:
            puml_once(plantuml, args.FILES, out_dir)
            if not args.once:
